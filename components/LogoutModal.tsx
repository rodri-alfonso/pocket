			...
			{isOpen && (
				<Modal isOpen={isOpen} onClose={() => setIsOpen(false)} className='w-4/6 h-auto relative'>
					...
					<div className='grid justify-center h-full gap-6 py-6 w-4/5 mx-auto'>
						<span className='text-center font-semibold text-xl'>Settings</span>
						<div className='w-56 font-semibold'>
							<button
								className='mb-3 bg-gray-800 border-2 border-solid border-gray-800 hover:bg-white hover:text-gray-800 text-white px-9 w-full py-2 rounded-full active:scale-95 transition-all'
								onClick={deleteRegistration}
							>
								Log out
							</button>
							<button
								className='border border-solid hover:border-gray-200 hover:underline hover:text-gray-700 border-gray-300 text-gray-500 px-9 py-2  w-full rounded-full active:scale-95 transition-all'
								onClick={() => setIsOpen(false)}
							>
								Delete account
							</button>
